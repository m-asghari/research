	...
	public static void RunExperiment(int startHour, Util.PredictionMethod predictionMethod, int predictionTime) {
		...
		try {
			...
			for (int i = 0; i < k; ++i) {
				...
				PMF modelDist1, modelDist2, modelDist3;
				Double actualTime;
				...
				case NoPrediction:
					for (Calendar startTime : testDays) {
						...
						modelDist1 = Approach2.GenerateModel(sensorList, null, null, queryTime);
						if (modelDist1 == null) continue;
						actualTime = Approach2.GenerateActual(sensorList, (Calendar)startTime.clone());
						if (actualTime == null) continue;
						Double score = modelDist1.GetScore(actualTime);
						totalCRPSScore += score;
						totalEVScore += Math.abs(modelDist1.mean - actualTime);
						totalCount++;
					}
					break;
				case Historic:					
					...
					ArrayList<Double> actualTimes = new ArrayList<Double>();
					modelDist1 = Approach2.GenerateModel(sensorList, timeOfDay, modelDays, null);
					modelDist2 = Approach4.GenerateModel(sensorList, timeOfDay, modelDays, null);
					if (modelDist1 == null) {
						//Util.no_model++;
						continue;
					}
					...
					for (Calendar startTime : testDays) {
						...
						Double score1 = modelDist1.GetScore(actualTime);
						Double score2 = modelDist2.GetScore(actualTime);
						//Util.Log(String.format("Approach2 Mean:%f\tActual Time:%f", modelDist1.mean, actualTime));
						//Util.Log(String.format("Approach2 CRPS score: %f\tApproach2 EV score: %f", score1, Math.abs(modelDist1.mean - actualTime)));
						//Util.Log(String.format("Approach4 Mean:%f\tActual Time:%f", modelDist2.mean, actualTime));
						//Util.Log(String.format("Approach4 CRPS score: %f\tApproach4 EV score: %f", score2, Math.abs(modelDist2.mean - actualTime)));
						totalCRPSScore += score1;
						totalEVScore += Math.abs(modelDist1.mean - actualTime);
						totalCount++;
					}
					modelDist3 = new PMF(actualTimes);
					Util.Log("Approach2 Model: " + modelDist1.toString());
					Util.Log("Approach4 Model: " + modelDist2.toString());
					Util.Log("Actal Time Model: " + modelDist3.toString());
					break;
				case Filtered:
					for (Calendar startTime : testDays) {
						...
						modelDist1 = Approach2.GenerateModel(sensorList, timeOfDay, modelDays, (Calendar)queryTime.clone());
						if (modelDist1 == null) continue;
						actualTime = Approach2.GenerateActual(sensorList, (Calendar)startTime.clone());
						if (actualTime == null) continue;
						Double score = modelDist1.GetScore(actualTime);
